...
zcenternewtemp3=Delete[ReplacePart[zcenter1,ntot->zcenter1[[i]]],{{i}}];
  ...
  s5 = OpenWrite[
    "~/Dropbox/Mathematica/ObliqueFiniteArray/Fracture/1Inclusion/Package/output/output"];
  Write[s5, n];
  Write[s5, num1];
  Write[s5, num2];
  Write[s5, ntot -1];
...
readstart[]:=Block[{s8,rtest},s8 = OpenRead[
   "~/Dropbox/Mathematica/ObliqueFiniteArray/Fracture/1Inclusion/Package/output/start"];
...
kk = If[ntot == ninitial, SIF`kcrack[p, p], 
     SIF`kcrack[p - ttest, p - ttest + 1]];
PutAppend[List[Re[kk],Im[kk],g1,g2,f1],"~/Dropbox/Mathematica/ObliqueFiniteArray/Fracture/1Inclusion/Package/output/crackenergy"];
