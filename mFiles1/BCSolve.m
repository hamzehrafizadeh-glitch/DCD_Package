...
Do[Sij[[i (*+NTOTAL ****** if both XY are fixed ******* *),j]]=Uij[[i,j]],{i,jjn},{j,1,2*NTOTAL}];
Do[Sij[[i+NTOTAL,j]]=Uij[[i+NTOTAL,j]],{i,jjs},{j,1,2*NTOTAL}]; (*   ***** if both XY are fixed ***** *)  




Clear[TnfieldNormalize,TsfieldNormalize]
TnfieldNormalize[t_]:=Block[{f,S11,S22,S12},f=Simplify[(S11+S22)/2. +Cos[2*t](S11-S22)/2.  +S12*Sin[2*t]];
S11:=Xload*Normalize[s11];
S22:=Xload*Normalize[s22];
S12:=Xload*Normalize[s12];
f]

TsfieldNormalize[t_]:=Block[{f,S11,S22,S12},f=Simplify[-Sin[2*t](S11-S22)/2.  +S12*Cos[2*t]];
S11:=Xload*Normalize[s11];
S22:=Xload*Normalize[s22];
S12:=Xload*Normalize[s12];
f]
...
SFar1=Table[t=-XY[[j,4]]+Pi/2;TnfieldNormalize[t],{j,NTOTAL}];

SFar2=Table[t=-XY[[j,4]]+Pi/2;TsfieldNormalize[t],{j,NTOTAL}];
SFar=Join[SFar1,SFar2];*)


Do[SFar[[i]] = 0.5, {i, {1, 2,3,4,n1+1,n1+2,n1+3,n1+4}}];
(*Do[SFar[[i]] = 1, {i, 1,2n1}]*)
...
If[ncorner!=0,SFar[[2NTOTAL-2]]=0Xload;
SFar[[NTOTAL-2]]=0Xload]

SFar[[n1+1]]=1*)



(*SFar[[2NTOTAL-3]]=0; and SFar[[n1+1]]=0;*)



Print["jjn =   ",jjn]
Table[SFar[[i]]=0,{i,jjn}]
If[ncorner!=0,SFar[[NTOTAL-1]]=0;
SFar[[2*NTOTAL-1]]=0;
SFar[[NTOTAL-2]]=0;
SFar[[2*NTOTAL-2]]=0]
