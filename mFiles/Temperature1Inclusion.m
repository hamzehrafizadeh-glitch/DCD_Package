...
gamma=50.+I*20;
Fn[q_]:={dd[q]*Conjugate[gamma]/2.}
ReFn[q_,n_]:= Module[{f}, f= Re[Fn[q][[n]]]];
ImFn[q_,n_]:= Module[{f}, f= Im[Fn[q][[n]]]];
lengthFn[q_] := Length[Fn[q]];
...
f= ReFn[q,n]*(1+f1*f3/f2)/ThermalConductivityQBar[q]];
ImGnq[n_,q_] :=Module[{f,f1,f2,f3},
f1=((ThermalConductivityQBar[q]-1)*(1-v0[q]^(2n)));
f2= (ThermalConductivityQBar[q]+(v0[q]^(n)-v0[q]^(-n))/(v0[q]^(n)+v0[q]^(-n)));
f3=(-1+v0[q]^(-2n))/(v0[q]^(2n)-v0[q]^(-2n));
f= ImFn[q,n]*(1+f1*f3/f2)/ThermalConductivityQBar[q]]
...
FnFinal[q_]:=Table [If[i < (lengthFn[q]+1),Fn[q][[i]],0.],{i,n}] 
SnqFinal[q_]:=Table [If[i < (lengthFn[q]+1),Resnq[i,q] +I*Imsnq[i,q],0.],{i,n}]
GnqFinal[q_]:=Table [If[i < (lengthFn[q]+1),ReGnq[i,q] +I*ImGnq[i,q],0.],{i,n}]
