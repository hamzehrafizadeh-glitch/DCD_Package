...
Unprotect[bcTem1nq,bcTem2nq, Fn, Snq, STem, gamma,Bettal0,BettalQ]
...
bcf1[i_, j_,q_] := Module[{f,f1},
f1=-Chi0 listaa[q][[j]] + v0[q]^(2 j)Conjugate[listbbminus[q][[j]]];
f= If[TS==0,f1,f1+ bcTem1nq[j,q]];
If[i == 1,f,0]
]

bcf2[i_, j_,q_] := Module[{f,f1},
f1 =Chi0 v0[q]^(2j) listaaminus[q][[j]]- Conjugate[listbb[q][[j]]];
f= If[TS==0,f1,f1 + bcTem2nq[j,q]];
If[i == 2,f,0]
]
...
BeginPackage["farfield`",{"geometry`","ExpansionCoefficient`","abmnpq`","VectorMatrixForms`" ,"TemperatureInclusions`"}] 
EndPackage[] 
