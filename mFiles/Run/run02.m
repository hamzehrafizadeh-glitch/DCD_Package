...
UnFields[t_]:=Block[{f,uxt,uyt},f=Simplify[uxt Cos[t]+uyt Sin[t]];
uxt:=Re[ufarzt[ntot]];
uyt:=Im[ufarzt[ntot]];
f]

UsFields[t_]:=Block[{f,uxt,uyt},f=Simplify[uyt Cos[t]-uxt Sin[t]];
uxt:=Re[ufarzt[ntot]];
uyt:=Im[ufarzt[ntot]];
f]
...
SUnMM := Block[{r, f,i,j,t}, r :=Simplify[ Sum[Un[t,i], {i, 1, ntot}]+UnFields[t]];
  f = Table[x = Re[point[[j]]]; y = Im[point[[j]]];t=-XY[[j,4]]+Pi/2; If[Snn[[j]]==1, 0,r], {j, 1, NTOTAL}]]

SUsMM := Block[{r, f,i,j,t}, r :=Simplify[ Sum[Us[t,i], {i, 1, ntot}]+UsFields[t]];
...
Table[ufarzt[i]=ufarz[i],{i,1,ntot}];
Table[uxzt[i]=u0xz[i](*-Re[ufarzt[i]]*),{i,1,ntot}];
Table[uyzt[i]=u0yz[i](*-Im[ufarzt[i]]*),{i,1,ntot}];
...
SMM = SUMM +SSMM; 
(*Table[If[Abs[SMM[[i]]]>1.9,SMM[[i]]=0],{i,1,2NTOTAL}]*)
