...
(*(*Pure Tension *)
SyMM := Block[{r, f,i,j,t}, r :=Simplify[ Sum[Tn[t,i], {i, 1, ntot}]+Tnfield[t]];
  f = Table[x = Re[point[[j]]]; y = Im[point[[j]]];t=If[1<=j<=2n1,Pi/2,0]; r, {j, 1, NTOTAL}]]

TxyMM := Block[{r, f,i,j,t}, r :=Simplify[ Sum[Ts[t,i], {i, 1, ntot}]+Tsfield[t]];
  f = Table[x = Re[point[[j]]]; y = Im[point[[j]]];t=If[1<=j<=2n1,Pi/2,0]; r, {j, 1, NTOTAL}]]*)


(*Displacement fixed for points j\[Equal]2n1+n2+1||j\[Equal]2n1+2n2*)
(*SyMM := Block[{r, f,i,j,t}, r :=Simplify[ Sum[Tn[t,i], {i, 1, ntot}]+Tnfield[t]];
  f = Table[x = Re[point[[j]]]; y = Im[point[[j]]];t=If[1<=j<=2n1,Pi/2,0]; If[j==2n1+n2+1||j==2n1+2n2,0,r], {j, 1, NTOTAL}]]
...
SnMM := Block[{r, f,i,j,t}, r :=Simplify[ Sum[Tn[t,i], {i, 1, ntot}]+Tnfield[t]];
  f = Table[x = Re[point[[j]]]; y = Im[point[[j]]];t=-XY[[j,4]]+Pi/2; If[j==2n1+n2+1||j==2n1+2n2,0,r], {j, 1, NTOTAL}]]

SsMM := Block[{r, f,i,j,t}, r :=Simplify[ Sum[Ts[t,i], {i, 1, ntot}]+Tsfield[t]];
  f = Table[x = Re[point[[j]]]; y = Im[point[[j]]];t=-XY[[j,4]]+Pi/2; If[j==2n1+2n2,0,r], {j, 1, NTOTAL}]]
...
SUnMM := Block[{r, f,i,j,t}, r :=Simplify[ Sum[Un[t,i], {i, 1, ntot}]];
  f = Table[x = Re[point[[j]]]; y = Im[point[[j]]];t=-XY[[j,4]]+Pi/2; If[j==2n1+n2+1||j==2n1+2n2,r,0], {j, 1, NTOTAL}]]

SUsMM := Block[{r, f,i,j,t}, r :=Simplify[ Sum[Us[t,i], {i, 1, ntot}]];
  f = Table[x = Re[point[[j]]]; y = Im[point[[j]]];t=-XY[[j,4]]+Pi/2; If[j==2n1+2n2,r,0], {j, 1, NTOTAL}]]
