...
Sigma22ztot::usage=" Sigma22ztot is the total Sigma11 stresses in the matrix material at z coordinate system. "


Sigma11zQ::usage=" Sigma11zQ[q_] is the Sigma11 stress field inside the inclusion q at z coordinate system. "
Sigma12zQ::usage=" Sigma12zQ[q_] is the total Sigma11 stress field inside the inclusion q at z coordinate system. "
Sigma22zQ::usage=" Sigma22zQ[q_] is the total Sigma11 stress field inside the inclusion q at z coordinate system. "
...
(*Sigma11ztot, Sigma12ztot, Sigma2ztot are total stresses at z coordinate system. *)
Clear[Sigma11ztot,Sigma12ztot,Sigma22ztot]
Sigma11ztot:=Module[{f},f=Re[Alpha0z]+s11]
Sigma12ztot:=Module[{f},f=-Im[Alpha0z]+s12]
Sigma22ztot:=Module[{f},f=Beta0z-Re[Alpha0z]+s22]



(*Sigma11zQ, Sigma12zQ, Sigma2zQ are stresses at z coordinate system inside the inclusion q. *)
Print["CHECK IF I HAVE TO ADD FARFIELDS?"]
...
Sigma11m[q_]:=Module[{f,f1,f2,xt,yt},f1=If[RS==1,f2=SigmaxxYq[q,xt,yt];xt=Re[dd[q]Cosh[Xi]];yt=Im[dd[q]Cosh[Xi]];
f2,0];f=Re[AlphaFar[q]+Alpha0[q]]+Re[f1]]

Sigma12m[q_]:=Module[{f,f1,f2,xt,yt},f1=If[RS==1,f2=SigmaxyYq[q,xt,yt];xt=Re[dd[q]Cosh[Xi]];yt=Im[dd[q]Cosh[Xi]];
f2,0];
f=-Im[AlphaFar[q]+Alpha0[q]]+Re[f1]]

Sigma22m[q_]:=Module[{f,f1,f2,xt,yt},f1=If[RS==1,f2=SigmayyYq[q,xt,yt];xt=Re[dd[q]Cosh[Xi]];yt=Im[dd[q]Cosh[Xi]];
f2,0];
...
Clear[zp,zq,Xi,z]
Sigma11z[i_]:=Module[{f,f1,f2,f3},f1=If[RS==1,f2=SxxFinalTemp[i,xt,yt];xt=Re[dd[i]Cosh[Xi]];yt=Im[dd[i]Cosh[Xi]];
...
Sigma12z[i_]:=Module[{f,f1,f2,f3},f1=If[RS==1,f2=TxyFinalTemp[i,xt,yt];xt=Re[dd[i]Cosh[Xi]];yt=Im[dd[i]Cosh[Xi]];
f2,0];f3=Sigma12mStresslist[[i]] Cos[2 Theta[i]]+(Sigma11mStresslist[[i]]-Sigma22mStresslist[[i]]) Cos[Theta[i]] Sin[Theta[i]];
f=f3+Re[f1]];

Sigma22z[i_]:=Module[{f,f1,f2,f3},f1=If[RS==1,f2=SyyFinalTemp[i,xt,yt];xt=Re[dd[i]Cosh[Xi]];yt=Im[dd[i]Cosh[Xi]];
...
BeginPackage["StressFields`",{"geometry`", "ExpansionCoefficient`" , "abmnpq`" , "matrix`" ,"TemperatureInclusions`", "farfield`" ,"linearSolve`", "PhiPsi`","CombinDCDTEMP`"}] 
EndPackage[] 
